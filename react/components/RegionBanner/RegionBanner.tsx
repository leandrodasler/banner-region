...
const RegionBanner = ({ images, height = 420, children, preload }: PropsWithChildren<ImageListProps>) => {
  const [filteredImages, setFilteredImages] = useState<ImagesSchema | null>(null);
  const handles = useCssHandles(CSS_HANDLES);

  useEffect(() => {
    const regionSelected = JSON.parse(localStorage.getItem("region-selected") as string);
    ...
    fetch("/api/sessions", {
      ...
      body: JSON.stringify({
        public: { country: { value: country }, postalCode: { value: postalCode } },
      }),
      method: "POST",
      ...
      .then((res) => {
        ...
        setFilteredImages(
          images
            ? images.filter((image) => {
                const franchises = image.franchises && !/^\s*$/.test(image.franchises) ? image.franchises?.trim().split(/\s*,\s*/) : null;
                if (!franchises) {
                  return true;
                }
  ...
  return <ImageList images={filteredImages} height={height} children={children} preload={preload} />;
};
